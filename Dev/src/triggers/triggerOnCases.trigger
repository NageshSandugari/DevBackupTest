     ...
     // Added to update SLA Type Field support ticket #08951763                   
       ...
       */
           // System.debug('Case Details: ' + c.CaseNumber + ', '+ c.Status + ', ' + c.RecordType.Name + ', ' + c.Type + ', ' + c.Case_Record_Type__c +', ' + c.Service_request_Type__c);
            if (String.isBlank(c.description) && c.Type =='Service Request Type' ) {        
             c.Description = '.';       
...
//            else if(c.Case_Record_Type__c == 'Construction Site Task Force')  { // FIX for SR#10848060
            else if(c.Case_Record_Type__c == 'Building Force')  {
            /* Commenting as per new requirements Case Number 05012710
            ...
            /*Commenting as per new requirements Case Number 05012710       
                ...
                //Added for ticket #10660908 START
                
                if(c.Is_this_a_donation_bin__c==NULL || c.Is_this_a_donation_bin__c=='' || c.Is_this_a_donation_bin__c=='No' )
                {
                    if(c.Residential_or_Commerical__c=='Residential')
                 ...
                 else if(c.Is_this_a_donation_bin__c=='Yes' && c.On_private_property__c=='Yes')
                 {
                        c.Service_Request_Type__c = 'Maintenance Residential';  
                 }
                 else
                 {
                        c.Service_Request_Type__c = 'Donation Bin';
                 }
                 //Added for ticket #10660908 END
            } 
          ...
          /*  else if(c.Case_Record_Type__c == 'Zoning Residential')  {
            ...
            /* Removing this functionality for Related cases change 10577449 - START - UNISYS
            ...
            if(c.parentId != null)    {
                ...
                if (c.Status != 'Closed' && c.Redress_Case__c == FALSE && c.Type != null){
                    if(c.Service_Request_Type__c != 'Recyclables Collection' && c.Service_Request_Type__c != 'Dumpster Violation')
                        ...
                        if( !(pCase.Service_Request_Type__c == 'Rubbish Collection' && pCase.Type_of_Rubbish_Missed__c == 'Both' && pCase.Type_of_Rubbish_Missed__c == 'Both'))    {
                            c.Status = 'Closed';
                        }
                            
                    }
                    else if(c.Service_Request_Type__c == 'Dumpster Violation')    {
                        if(!( pCase.Service_Request_Type__c == 'Sanitation Violation' && pCase.Violation_Type__c == 'Both' && pCase.Violation_Type__c == 'Both'))
                            c.Status = 'Closed';
                    }
                } 
            }
            Removing this functionality for Related cases change 10577449 - END - UNISYS */ 
    ...
    //  Updated the code for Support ticket #00005373
        ...
        if(Trigger.isInsert)    {   
            if (!CaseFieldUpdate.inFutureContext) {
                for (Case c : Trigger.new) {
                    System.debug('Case Details: ' + c.CaseNumber + ', '+ c.Status + ', ' + c.RecordType.Name + ', ' + c.Type + ', ' + c.Case_Record_Type__c +', ' + c.Service_request_Type__c);
                    if(c.Validated_Cases_from_PS__c != TRUE)
                        CaseId.add(c.id);
                }    
         ...
         if(Trigger.isUpdate)    { 
            ...
            Map<id, String> parentCasesStatus = new Map<id, String>();
            List<Case> childCasesToUpdate = new List<Case>();
            
            //Gather up a set of the parent cases.
            for (Case c:Trigger.new) {
                    parentCaseIds.add(c.id);
                    parentCasesStatus.put(c.id,c.status);
                }
            System.debug('All Cases :' + parentCaseIds);   
                
            if (parentCaseIds.size()>0) {
                List<Case>  childCases = [Select id, CaseNumber, Status,parentId 
                                            From Case 
                                            Where parentId in :parentCaseIds];
                
                for (Case childCase:childCases) {
                    String parentStatus ;
                    parentStatus = parentCasesStatus.get(childCase.parentId);
                        //chnage the status, if it is not matching with parent case
                         if(childCase.status != parentStatus){
                            childCase.status = parentStatus;
                            childCasesToUpdate.add(childCase);
                            System.debug('These are the child cases :' + childCasesToUpdate); 
                         } 
                    }
        ...
        /* 
          //added by raju
         //Workflow implementation to synchronize case status and comments
           list<Case> upCas = new List<Case>();
           for (Case c : Trigger.new) {
           List<Case> cCase = [Select id, CaseNumber, Status from Case where parentId =: c.Id];            
           System.debug('These are the child cases :' + cCase);
           if(cCase != null) {
           System.debug('Entered child case if');
           for(Case cas : cCase) {
           System.debug('Running for');
           cas.status = c.status; 
           upCas.add(cas);     
            ...
            if (!CaseFieldUpdate.inFutureContext) {
                ...
                for (Case c : Trigger.new) {
                    ...
                    system.debug('old case is'+oldCase);
                     //List<Group> g = [Select id, Name from Group where id =: c.OwnerId LIMIT 1];   Commented By Nagesh  
                    String ownerName = '';    
                    for( Group each :groups ){
                        if(each.id == c.OwnerId){
                        ownerName = each.Name;
                        }
                    }
                    /*String ownerName = '';     commented By Nagesh           
                    ...
                    if(ownerName != null && ownerName == '311 Mobile Requests' && c.Validated_Cases_from_PS__c == FALSE)
                        CaseId.add(c.id); 
                    else if(c.Department__c != 'Streets Department' && c.Department__c != 'Water Department (PWD)' && c.Validated_Cases_from_PS__c != TRUE)    {
                        //change for support ticket #10906587 START 
                        if((c.Status == 'Closed' && c.Status!=oldcase.Status )|| c.Service_Request_Type__c  != oldCase.Service_Request_Type__c )
                            CaseId.add(c.id);
                        // Updated the condition as per Support Ticket# 09171194     
                        else if(ownerName != null && ownerName == 'CitiWorks Reject' && c.Centerline_2272x__c != null && c.Centerline_2272x__c != 0 && c.Centerline_2272x__c != 0.0)
                            CaseId.add(c.id);
                    }
                    else if(c.Validated_Cases_from_PS__c != TRUE) {        
                        if(ownerName != null && ownerName == 'CitiWorks Reject' && c.Centerline_2272x__c != null && c.Centerline_2272x__c != 0 && c.Centerline_2272x__c != 0.0)
                            CaseId.add(c.id);  
                       // else if((c.Status == 'Closed' && c.Status!=oldcase.Status )|| c.Service_Request_Type__c  != oldCase.Service_Request_Type__c )
                         //   CaseId.add(c.id);       
                        //change for support ticket #10906587 END                  
                    }        
                }
