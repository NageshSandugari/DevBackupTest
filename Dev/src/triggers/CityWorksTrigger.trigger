trigger CityWorksTrigger on Case (before insert,before update, before delete, after update, after insert) {
    ...
    Group gr = [Select Id from Group WHERE Name = '311 Mobile Requests' LIMIT 1];
 //   Contact newCon =[Select id, Name from Contact where name = 'ANON ANON' LIMIT 1];
    Id conctId = '0031k000001tKs9AAE';  
         ...
         //  if(Trigger.isInsert )    {
                ...
                for(Case cs:Trigger.new) {      
                   ...
                   */
                   ...
                   // Removing '<', '>' from description field for Support Ticket #09171194 
                   // support ticket fix 09274814 start
                   String specialChars = '<|>\"|[|]';
                   // support ticket fix 09274814 end
                   if(cs.Description != null && cs.Description != '' )  {
                       if(cs.Description.length() > 10000)
                           cs.Description = cs.Description.subString(0,9999);
                       for(integer j=0; j<specialChars.split('|').size(); j++)
                           cs.Description = cs.Description.replace(specialChars.split('|')[j], '');
                   }
                       
                   // Removing the text 'Address verification not done' from the description, if the address is verified
                   if((cs.Description != null && cs.Description != '' ) && (cs.Centerline_2272x__c != 0.0) && (cs.Centerline_2272x__c != NULL) && (cs.Description.contains('ADDRESS VERIFICATION NOT DONE'))) {
                        if(cs.Description.contains('ADDRESS VERIFICATION NOT DONE')) {
                   ...
                   //Group gr = [Select Id from Group WHERE Name = '311 Mobile Requests' LIMIT 1]; Moved out of loop for DataLoader.
                   
                   if(trigger.isInsert) {
                   if(cs.Customer_Request_Id__c != null)
                       i.parseDetailsFieldfromPS(cs, gr); 
                   }
                   // CityWorks Integration
                   if(cs.Department__c == 'Streets Department' || cs.Department__c == 'Water Department (PWD)')    {
                      
                       i.updateDetailsField(cs, gr);
                   }
                    ...
                    // Added Code for Hansen integration
                       ...
                       //Fix For Support Ticket #08993566 
                        if(cs.contactId == null && userInfo.getUserType() == 'Standard'  && cs.Case_Record_Type__c != 'LI Escalation' )  {
                          //  Contact newCon =[Select id, Name from Contact where name = 'ANON ANON' LIMIT 1]; Moved out of loop for DataLoader.
                            // added to sync with standards
                            cs.Customer_Declined__c = false;
                            //if(newCon != null)
                            cs.contactId = '0031k000001tKs9AAE';
                        }
                       ...
                       /* else if(cs.Case_Record_Type__c == 'Building Force'){
                            if(cs.Building_Collapsing__c != null) 
                                details +=  ' Building Collapsing : ' + cs.Building_Collapsing__c + '. ';
          ...
          //  System.debug(Trigger.new);
            ...
            /* REMOVING FOR SUPPORT TICKET #11698741
